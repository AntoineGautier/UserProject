...
model VAV_AHU
  ...
  UserProject.AirHandlersFans.VAV_AHU_Config1 VAV_AHU_1(
    ...
    final dat=datAll.VAV_AHU[1],
    redeclare final package MediumAir = MediumAir,
    redeclare final package MediumChiWat = MediumChiWat,
    final energyDynamics=energyDynamics) "Air handling unit"
  ...
  Buildings.Fluid.Sensors.Pressure pBui(redeclare final package Medium = MediumAir)
  ...
  Buildings.Templates.AirHandlersFans.Validation.UserProject.VAVBoxControlPoints
    sigVAVBox[VAV_AHU_1.nZon](each final stdVen=datAll.stdVen) if VAV_AHU_1.ctl.typ
  ...
  Buildings.Templates.ZoneEquipment.Validation.UserProject.BASControlPoints
    sigBAS(final nZon=VAV_AHU_1.nZon) "BAS control points"
    annotation (Placement(transformation(extent={{-100,80},{-80,100}})));
  Buildings.Templates.ZoneEquipment.Validation.UserProject.ZoneControlPoints
    sigZon[VAV_AHU_1.nZon] "Zone control points"
    annotation (Placement(transformation(extent={{-100,50},{-80,70}})));
protected
  Buildings.Templates.AirHandlersFans.Interfaces.Bus busAHU
  "Gateway bus"
  annotation (
    Placement(
...
equation
  connect(bouHeaWat.ports[1], VAV_AHU_1.port_aHeaWat) annotation (Line(points={{-80,-71},
          {-5,-71},{-5,-50}},           color={0,127,255}));
  connect(bouChiWat.ports[1], VAV_AHU_1.port_bChiWat)
    annotation (Line(points={{-80,-101},{5,-101},{5,-50}},
                                                         color={0,127,255}));
  connect(VAV_AHU_1.port_bHeaWat, bouHeaWat.ports[2]) annotation (Line(points={{-13,-50},
          {-13,-68},{-80,-68},{-80,-69}},           color={0,127,255}));
  connect(VAV_AHU_1.port_aChiWat, bouChiWat.ports[2]) annotation (Line(points={{13,-50},
          {13,-102},{-80,-102},{-80,-99}},           color={0,127,255}));
  connect(bouOut.ports[1], res.port_a) annotation (Line(points={{-80,-31},{-60,-31},
          {-60,-40},{-50,-40}}, color={0,127,255}));
  connect(res.port_b, VAV_AHU_1.port_Out)
    annotation (Line(points={{-30,-40},{-20,-40}}, color={0,127,255}));
  connect(VAV_AHU_1.port_Sup, res1.port_a)
  ...
  connect(weaDat.weaBus, VAV_AHU_1.busWea) annotation (Line(
      points={{-80,0},{0,0},{0,-10}},
      color={255,204,51},
      thickness=0.5));
  connect(busAHU, VAV_AHU_1.bus) annotation (Line(
      points={{-20,10},{-20,-14},{-19.9,-14}},
      color={255,204,51},
      thickness=0.5));
  connect(VAV_AHU_1.port_Rel, res2.port_a)
    annotation (Line(points={{-20,-20},{-30,-20}}, color={0,127,255}));
  connect(res2.port_b, bouOut.ports[2]) annotation (Line(points={{-50,-20},{-60,
          -20},{-60,-29},{-80,-29}}, color={0,127,255}));
  connect(VAV_AHU_1.port_Ret, res3.port_b)
    annotation (Line(points={{20,-20},{30,-20}}, color={0,127,255}));
  ...
  connect(sigVAVBox.bus, VAV_AHU_1.busTer) annotation (Line(
      points={{-80,30},{19.8,30},{19.8,-14}},
      color={255,204,51},
      thickness=0.5));
  connect(sigBAS.busTer, VAV_AHU_1.busTer) annotation (Line(
      points={{-80,90},{19.8,90},{19.8,-14}},
      color={255,204,51},
      thickness=0.5));
  connect(sigZon.bus, VAV_AHU_1.busTer) annotation (Line(
...
<code>Buildings/Resources/Scripts/travis/templates/VAVMultiZone.py</code>.
</p>
</html>"),
